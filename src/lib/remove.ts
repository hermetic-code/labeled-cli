...
export default function remove(label: string) {
    ...
    for (const manager of Object.keys(data[label])) {
        ...
        logger.info(`Queued Environment`, `${manager} (${pkgs.length} packages)`);
    ...
    try {
        ...
        logger.info('System Purge', 'Sweeping away unreferenced orphan packages');
        console.log('\n');
        ...
        logger.error('An unexpected breakdown occurred during system cleanup', e);
